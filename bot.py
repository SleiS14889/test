...
# Текст кнопки, яка дублює команду /start
START_BUTTON_TEXT = "🐾 Пройти анкету"


def main_menu_keyboard():
    """Постійна клавіатура з кнопкою запуску анкети замість /start."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardRemove(),  # ховаємо кнопку на час анкети
    )
    return OWNER_NAME
...
async def finish_and_save(update: Update, context: ContextTypes.DEFAULT_TYPE, comment: str):
    ...
    await update.effective_message.reply_text(
        ...
        "Щоб пройти анкету знову — натисніть кнопку нижче.\n"
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.clear()
    await update.message.reply_text(
        "Анкету скасовано. Натисніть кнопку нижче, щоб почати знову.",
        reply_markup=main_menu_keyboard(),
    )
    return ConversationHandler.END
...
def main():
    ...
    start_trigger = filters.TEXT & filters.Regex(f"^{START_BUTTON_TEXT}$")

    conv_handler = ConversationHandler(
        entry_points=[
            CommandHandler("start", start),
            MessageHandler(start_trigger, start),
        ],
        states={
